    ...
    <x-card-container>
        <form action="{{ route('admin.booking.store') }}" method="POST">
            ...
            <div class="grid grid-cols-2 gap-x-3">
                <x-input id="name" label="Nama Acara" placeholder="Masukan nama" name="name" type="text" />
                <x-input-single-datepicker label="Tanggal booking" id="booking_date" name="booking_date" autocomplete="off" />
            </div>
            <div class="grid grid-cols-2 gap-x-3">
                <x-input id="total_person" label="Jumlah Tamu" name="total_guest" type="number" />
                <x-input id="time" label="Waktu" placeholder="Masukan waktu" name="time" type="time" />
            </div>
            ...
            <div class="lg:flex gap-x-3">
                <div class="lg:w-2/3">
                    <div class="grid sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @forelse ($menus as $menu)
                            <div onclick="addCart({{ $menu->id }} , {{ $menu->price }})"
                                class="w-full bg-white hover:border h-fit hover:border-white-300 rounded-2xl shadow-xl hover:shadow-2xl">
                                <a href="#">
                                ...
                                <div class="px-5 pb-5 mt-4">
                                    ...
                                    <div class="flex items-end justify-between">
                                        ...
                                        <a href="#"
                                            ...
                                            <span class="hidden md:inline-block">Pesan</span>
                                        </a>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="lg:w-1/3 p-4 hidden" id="detailOrder">
    ...
    <label for="confirmOrderModal" class="modal cursor-pointer">
        <label class="modal-box relative" for="">
            ...
            <div class="modal-action">
                ...
                <x-button color="green" onclick="btnOrder()">Konfirmasi
                    Pesanan</x-button>
            </div>
        </label>
    </label>
        ...
        <script>
            ...
            function btnOrder() {
                ...
                cart.forEach(function(item) {
                    ...
                    menuHtml += '<span class="flex-shrink-0">' + item.name + ' x ' + item.quantity + '</span>';
            ...
            function removeCart(id) {
                // reduce total price, remova cart item, and remove from array
                let price = parseInt($('#cart-item-' + id).data('price'));
                let quantity = parseInt($('#cart-item-' + id).data('quantity'));
